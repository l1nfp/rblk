...
static int client_prepare_connection(struct sockaddr_in *s_addr)
{
	...
	if (!conn->cm_event_channel)
	{
		rdma_error("Creating cm event channel failed, errno: %d \n", -errno);
		return -errno;
	}
	debug("RDMA CM event channel is created at : %p \n", conn->cm_event_channel);
	/* rdma_cm_id is the connection identifier (like socket) which is used
	 * to define an RDMA connection.
	 */
	ret = rdma_create_id(conn->cm_event_channel, &conn->id,
						 NULL,
						 RDMA_PS_TCP);
	if (ret)
	{
	 ...
	 * to a local device. */
	ret = rdma_resolve_addr(conn->id, NULL, (struct sockaddr *)s_addr, 2000);
	if (ret)
	{
	...
	ret = process_rdma_cm_event(conn->cm_event_channel,
	...
	ret = rdma_resolve_route(conn->id, 2000);
	if (ret)
	{
	 ...
	 */
	conn->io_completion_channel = ibv_create_comp_channel(conn->id->verbs);
	if (!conn->io_completion_channel)
	{
		rdma_error("Failed to create IO completion event channel, errno: %d\n",
				   -errno);
		return -errno;
	}
	debug("completion event channel created at : %p \n", conn->io_completion_channel);
	 ...
	 * is called "work" ;)
	 */
	conn->cq = ibv_create_cq(conn->id->verbs /* which device*/,
							 CQ_CAPACITY /* maximum capacity*/,
							 NULL /* user context, not used here */,
							 conn->io_completion_channel /* which IO completion channel */,
	...
	qp_init_attr.recv_cq = conn->cq; /* Where should I notify for receive completion operations */
	qp_init_attr.send_cq = conn->cq; /* Where should I notify for send completion operations */
	/*Lets create a QP */
	ret = rdma_create_qp(conn->id /* which connection id */,
						 conn->pd /* which protection domain*/,
...
static int client_pre_post_recv_buffer()
{
	...
	conn->server_metadata_mr = rdma_buffer_register(conn->pd,
													&conn->server_metadata_attr,
													sizeof(conn->server_metadata_attr),
													(IBV_ACCESS_LOCAL_WRITE));
	if (!conn->server_metadata_mr)
	{
		rdma_error("Failed to setup the server metadata mr , -ENOMEM\n");
		return -ENOMEM;
	}
	conn->client_recv_sge.addr = (uint64_t)conn->server_metadata_mr->addr;
	conn->client_recv_sge.length = (uint32_t)conn->server_metadata_mr->length;
	conn->client_recv_sge.lkey = (uint32_t)conn->server_metadata_mr->lkey;
	/* now we link it to the request */
	bzero(&conn->client_recv_wr, sizeof(conn->client_recv_wr));
	conn->client_recv_wr.sg_list = &conn->client_recv_sge;
	conn->client_recv_wr.num_sge = 1;
	ret = ibv_post_recv(conn->id->qp /* which QP */,
...
static int client_xchange_metadata_with_server()
{
	struct ibv_wc wc;
	int ret = -1;
	conn->client_src_mr = rdma_buffer_register(conn->pd,
											   src,
											   strlen(src),
											   (IBV_ACCESS_LOCAL_WRITE |
												IBV_ACCESS_REMOTE_READ |
												IBV_ACCESS_REMOTE_WRITE));
	if (!conn->client_src_mr)
	{
		rdma_error("Failed to register the first buffer, ret = %d \n", ret);
		return ret;
	}
	ret = process_work_completion_events(conn->io_completion_channel,
										 &wc, 1);
	if (ret != 1)
	{
	...
	show_rdma_buffer_attr(&conn->server_metadata_attr);
	return 0;
}
...
static int client_remote_memory_ops()
{
	...
	struct ibv_wc wc;
	int ret = -1;
	conn->client_dst_mr = rdma_buffer_register(conn->pd,
											   dst,
											   strlen(src),
											   (IBV_ACCESS_LOCAL_WRITE |
												IBV_ACCESS_REMOTE_WRITE |
												IBV_ACCESS_REMOTE_READ));
	if (!conn->client_dst_mr)
	{
	...
	/* now we fill up SGE */
	conn->client_send_sge.addr = (uint64_t)conn->client_src_mr->addr;
	conn->client_send_sge.length = (uint32_t)conn->client_src_mr->length;
	conn->client_send_sge.lkey = conn->client_src_mr->lkey;
	/* now we link to the send work request */
	bzero(&conn->client_send_wr, sizeof(conn->client_send_wr));
	conn->client_send_wr.sg_list = &conn->client_send_sge;
	conn->client_send_wr.num_sge = 1;
	conn->client_send_wr.opcode = IBV_WR_RDMA_WRITE;
	conn->client_send_wr.send_flags = IBV_SEND_SIGNALED;
	/* we have to tell server side info for RDMA */
	conn->client_send_wr.wr.rdma.rkey = conn->server_metadata_attr.stag.remote_stag;
	conn->client_send_wr.wr.rdma.remote_addr = conn->server_metadata_attr.address;
	/* Now we post it */
	ret = ibv_post_send(conn->id->qp,
						&conn->client_send_wr,
						&bad_client_send_wr);
	if (ret)
	{
	...
	conn->client_send_sge.addr = (uint64_t)conn->client_dst_mr->addr;
	conn->client_send_sge.length = (uint32_t)conn->client_dst_mr->length;
	conn->client_send_sge.lkey = conn->client_dst_mr->lkey;
	/* now we link to the send work request */
	bzero(&conn->client_send_wr, sizeof(conn->client_send_wr));
	conn->client_send_wr.sg_list = &conn->client_send_sge;
	conn->client_send_wr.num_sge = 1;
	conn->client_send_wr.opcode = IBV_WR_RDMA_READ;
	conn->client_send_wr.send_flags = IBV_SEND_SIGNALED;
	/* we have to tell server side info for RDMA */
	conn->client_send_wr.wr.rdma.rkey = conn->server_metadata_attr.stag.remote_stag;
	conn->client_send_wr.wr.rdma.remote_addr = conn->server_metadata_attr.address;
...
static int client_disconnect_and_clean()
{
	...
	ret = ibv_destroy_comp_channel(conn->io_completion_channel);
	if (ret)
	{
	...
	rdma_buffer_deregister(conn->server_metadata_mr);
	rdma_buffer_deregister(conn->client_src_mr);
	rdma_buffer_deregister(conn->client_dst_mr);
	/* We free the buffers */
	free(src);
	free(dst);
...
int main(int argc, char **argv)
{
	...
	if (ret)
	{
		rdma_error("Failed to prepost client receive buffer.\n");
		return ret;
	}
