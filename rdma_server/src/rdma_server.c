...
static int setup_client_resources()
{
	...
	if (!conn)
	{
		rdma_error("Failed to allocate memory for connection.\n");
		return ret;
	}
	...
	ret = ibv_req_notify_cq(conn->cq /* on which CQ */,
...
static int accept_client_connection()
{
	...
	memcpy(&remote_sockaddr /* where to save */,
		   rdma_get_peer_addr(conn->id) /* gives you remote sockaddr */,
...
static int send_server_metadata_to_client()
{
	...
	conn->rmem_mr = rdma_buffer_alloc(conn->pd /* which protection domain */,
									  DEFAULT_RMEM_SIZE /* what size to allocate */,
	 ...
	 */
	server_metadata_attr = calloc(1, sizeof(struct rdma_buffer_attr));
	server_metadata_attr->address = (uint64_t)conn->rmem_mr->addr;
	server_metadata_attr->length = (uint32_t)conn->rmem_mr->length;
	server_metadata_attr->stag.local_stag = (uint32_t)conn->rmem_mr->lkey;
	conn->server_metadata_mr = rdma_buffer_register(conn->pd /* which protection domain*/,
													server_metadata_attr /* which memory to register */,
													sizeof(struct rdma_buffer_attr) /* what is the size of memory */,
	 ...
	 * have one
	 */
	conn->server_send_sge.addr = (uint64_t)server_metadata_attr;
	conn->server_send_sge.length = sizeof(struct rdma_buffer_attr);
	conn->server_send_sge.lkey = conn->server_metadata_mr->lkey;
	/* now we link this sge to the send request */
	bzero(&conn->server_send_wr, sizeof(conn->server_send_wr));
	conn->server_send_wr.sg_list = &conn->server_send_sge;
	conn->server_send_wr.num_sge = 1;					 // only 1 SGE element in the array
	conn->server_send_wr.opcode = IBV_WR_SEND;			 // This is a send request
	conn->server_send_wr.send_flags = IBV_SEND_SIGNALED; // We want to get notification
	/* This is a fast data path operation. Posting an I/O request */
	ret = ibv_post_send(conn->id->qp /* which QP */,
						&conn->server_send_wr /* Send request that we prepared before */,
	...
	ret = process_work_completion_events(conn->io_completion_channel, &wc, 1);
	if (ret != 1)
	{
...
static int disconnect_and_cleanup()
{
	...
	rdma_buffer_free(conn->rmem_mr);
	rdma_buffer_free(conn->server_metadata_mr);
